  ...
  module Utility
    ...
    def start_unicorn
      %Q%
        ...
        cd #{fetch :app_path} && #{try_unicorn_user} RAILS_ENV=#{fetch :rails_env, :production} BUNDLE_GEMFILE=#{fetch :bundle_gemfile} #{fetch :unicorn_bundle} exec #{fetch :unicorn_bin} -c $UNICORN_CONFIG_PATH -E #{fetch :rails_env, :production} -D #{fetch :unicorn_options};
      %.split.join(' ')
    end
